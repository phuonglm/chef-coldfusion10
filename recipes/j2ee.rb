...
if node['cf11']['installer']['installer_type'] == "ear"
  ...
  execute "Explode ColdFusion 11 EAR" do
    ...
    COMMAND
    creates "#{node['cf11']['installer']['install_folder']}/cfusion.ear/cfusion.war"
    action :run
  end
  ...
  execute "Explode ColdFusion 11 WAR" do
    ...
    COMMAND
    creates "#{node['cf11']['installer']['install_folder']}/cfusion.ear/cfusion.war/META-INF/MANIFEST.MF"
    action :run
  end
  ...
  execute "Explode RDS WAR" do
    ...
    command <<-COMMAND
      ...
      mv rds rds.war
    COMMAND
    creates "#{node['cf11']['installer']['install_folder']}/cfusion.ear/rds.war/META-INF/MANIFEST.MF"
    action :run
  end
